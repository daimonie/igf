...
def calculate_energy(arguments): 
    ...
    for bias in biaswindow:
        ...
        for i in superset:
            if chances[i] > 1e-3:
                state           = calculation.ket(i)
                #print i, chances[i], state
                norm_squared    = np.dot(state.T, state)
                
                if norm_squared > 0:
                    energy          = np.dot(state.T, np.dot( calculation.epsilon, state))
                    interaction     = 0.00
                    
                    if state[0] == 1 and state[1] == 1:
                        interaction += float("Inf")
                    
                    this_energy = energy + interaction 
                    print "%d\t%.3e\t%d-%d\t%.3f"% (i, chances[i], state[0], state[1], energy + interaction) 
                    this_energy *= chances[i]
        energy_vector.append(this_energy)
    return [biaswindow, energy_vector]

biaswindow = np.linspace(bias_left, bias_right, bias_res) 
param_space = np.linspace(param_left,param_right,param_res)

manager = taskManager( cores, calculate_energy ) 
...
fig, ax1 = plt.subplots(sharex=True, figsize=(25, 15), dpi=1080)
plt.xticks(fontsize=30)
plt.yticks(fontsize=30) 
     
colors_plus = MaxNLocator(nbins=tick_num).tick_values(np.min(np.min(energy)), np.max(np.max(energy)))  

cf_plus = ax1.contourf(mesh_bias, mesh_param, energy, cmap=cmap, levels=colors_plus) 
...
ax1.set_xlabel( "Bias $V$" ,fontsize=30); 
...
#ksi = args.ksi
#zeta = args.zeta
plt.savefig("perrin_energy_parallel.pdf") 
